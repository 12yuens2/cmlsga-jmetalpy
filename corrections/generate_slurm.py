...
algorithms = ["omopso", "smpso", "cmpso", "heia", "unsgaiii", "moead", "ibea"]

def write_slurm_file(filename, output_path, logfilename, running):
    with open(filename, "w") as slurm_file:
        ...
        slurm_file.write("#SBATCH --time=57:05:00\n\n")
...
for algorithm in algorithms:
    for problem,array in problems.items():
        ...
        run_experiment = "python src/run_experiment.py {} {}${{SLURM_ARRAY_TASK_ID}} {}\n".format(algorithm, problem)

        if problem == "DASCMOP":
            for i in [5,6,7]:
                filename = "{}-{}-{}.slurm".format(algorithm, problem, i)
                logfilename = "smac300k-{}-{}-{}".format(algorithm, problem, i)
                run_experiment = "python src/run_experiment.py {} {}${{SLURM_ARRAY_TASK_ID}}\({}\) \n".format(algorithm, problem, i)

                write_slurm_file(filename, output_path, logfilename, run_experiment)
        else:
            write_slurm_file(filename, output_path, logfilename, run_experiment)
