...
algorithms = ["heia", "nsgaiii", "moead", "ibea", "omopso", "smpso", "cmpso"]
...
def write_slurm_file(filename, logfilename, running):
    with open(filename, "w") as slurm_file:
        ...
        slurm_file.write("#SBATCH --time=49:55:00\n\n")

        slurm_file.write("# Logging\n")
        slurm_file.write("#SBATCH --output=../logs/smac-{}.out\n".format(logfilename))
        slurm_file.write("#SBATCH --error=../logs/smac-{}.err\n".format(logfilename))
...
for algorithm in algorithms:
    for problem,array in problems.items():
        ...
        run_experiment = "python src/tuning.py {} {}${{SLURM_ARRAY_TASK_ID}} \n".format(algorithm, problem)
